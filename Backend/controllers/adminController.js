...
class AdminController {
    ...
    // ============================================
    // UPDATE LEARNER
    // Only name, surname, email, and phone are editable.
    // ============================================
    static async updateLearner(req, res) {
        try {
            ...
            console.log('Updating learner:', { id, name, surname, email, phone });
    ...
    // When status becomes 'Enrolled', insert into Enrolment table.
    // ============================================
            ...
            if (status === 'Enrolled') {
                try {
                    ...
                    return res.status(500).json({
                        success: false,
                        message: 'Failed to add learner to enrolment table: ' + enrolmentError.message
                    });
                }
            }
    ...
    // Skips pairs that already have a certificate.
    // ============================================
        ...
        try {
            ...
            for (const cert of certificates) {
                try {
                    ...
                    // Skip if this learner already has a certificate for this programme
                    const [existing] = await pool.execute(
                        `SELECT Certificate_id 
                         FROM Certificate 
                         WHERE User_id = ? AND Programme_id = ?`,
